...
public class AdvRoom /* extends AdvRoomStub */{
	...
	public String[] getDescription() {
		String[] d = new String[descriptions.size() + objects.size()];
		int i = 0;
		for (String s : descriptions) {
			d[i++] = s;
		}
		for (AdvObject o : objects) {
			d[i++] = "There is a " + o.getDescription() + " here.";
		}
		return d;
	}
	...
	public AdvMotionTableEntry[] getMotionTable() {
		return entries.toArray(new AdvMotionTableEntry[0]);
	}
	...
	public static AdvRoom readFromFile(Scanner scan) {
		...
		while (scan.hasNextLine()
			...
			room.descriptions.add(line);
		}
		while (scan.hasNextLine()
				&& (line = scan.nextLine().trim()).length() > 0) {
			String[] tokens = line.split("[/ ]+");
			String direction = tokens[0];
			int destinationRoom = Integer.parseInt(tokens[1]);
			String keyName = tokens.length == 2 ? null : tokens[2];
			room.entries.add(new AdvMotionTableEntry(direction,
					destinationRoom, keyName));
		}
		return room;
	...
	private List<AdvMotionTableEntry> entries = new ArrayList<AdvMotionTableEntry>();
}
